...
def auto():
    ...
    try:
        if not request.args.get("url"):
            return Response(
                json.dumps({"msg": "url parameter required"}, indent=4),
        ...
        for name, service in serv.items():
            try:
                ...
                if request.args.get("type") in ["direct", "embed"]:
                    for video in filter(lambda v: v.watermark and v.type == "video", b):
                        fvid = video.download()
                ...
                return Response(
                    json.dumps({"error": str(e)}, indent=4),
                    content_type="application/json",
                )
        return Response(
            json.dumps({"msg": "server busy"}, indent=4),
            content_type="application/json",
        )
    except Exception:
        return Response(
            json.dumps({"msg": "url is invalid"}, indent=4),
            content_type="application/json",
        )
    ...
    if path == "info":
        try:
            ...
            if request.args.get("legacy"):
                ...
                return Response(
                    json.dumps({"legacy": True, **resp.aweme}, indent=4),
                    content_type="application/json",
                )
            else:
    ...
    if request.args.get("url"):
        try:
            ...
            return Response(
                json.dumps(
                    [
                        {"type": str(i.type), "url": i.json, "watermark": i.watermark}
                        for i in res
                    ],
                    indent=4,
                ),
